...
bool SFGGraphLayoutUIController::doLayout(const QString &engine, CEditorScene &scene)
{
	...
	qDebug() << nodes.size() << " number of nodes";

	for (auto & node : nodes)
	{
		// unknown node is nr.
		nid = 0;
		// get the id as string of this node
		idstr = node->getId();
		// make a char * for sscanf
		cidstr = idstr.toLatin1();
		// node name begins with 'N' and starts with number 1
		status = sscanf (cidstr, "N%d", &nid);
		// check if 1 item correctly parsed
		if (status == 1) {
			// get x size
			...
			// should not happen
			std::printf ("could not get number of node %s\n", cidstr);
		}
		nodeMap[node] = nid;
	}
