...
{
// XXX this is depreciated    QStyleOptionViewItemV3 opt = option;
    QStyleOptionViewItem opt = option; // fixed
    bool hasValue = true;
    if (m_editorPrivate) {
	...
	// m_editorPrivate can be NULL
	// const
	QColor c;
	if (m_editorPrivate) {
		// m_editorPrivate->indexToBrowserItem(index) can be null
		if (m_editorPrivate->indexToBrowserItem(index)) {
...
// XXX this is depreciated    QStyleOptionViewItemV3 opt = option;
    ...
    if (index.column() == 1) {
        QTreeWidgetItem *item = (QTreeWidgetItem *)0;
	// can be null
	if(m_editorPrivate) {
		if (m_editorPrivate->indexToItem(index)) {
			item = m_editorPrivate->indexToItem(index);
		}
	}
...
void QtTreePropertyBrowserPrivate::updateItem(QTreeWidgetItem *item)
{
    ...
    bool isEnabled = wasEnabled; // never read
    Q_UNUSED(isEnabled);
    if (isEnabled == true) { }
    if (property->isEnabled()) {
