...
void QtFlagPropertyManagerPrivate::slotBoolChanged(QtProperty *property, bool value)
{
    ...
    while (itProp.hasNext()) {
        ...
        if (p == property) {
            ...
            if (value) {
// XXX todo
// XXX #warning "The result of the left shift is undefined due to shifting by '100', which is greater or equal to the width of type 'int' v |= (1 << level);"
                v |= (1 << level);
            } else {
// XXX also here
                v &= ~(1 << level);
        ...
        level++;
	// XXX fix?
	// if ((size_t)level >= 8*sizeof(int)) {
	//	qDebug() << "level overflow level=" << level;
	// }
    }
}
