...
static void reorg(struct gml_graph *g)
{
	...
	while (nl) {
		...
		if (nl->node->indegree == 0 && nl->node->outdegree == 0) {
			nn1 = calloc(1, sizeof(struct gml_nlist));
			if (nn1) {
				nn1->node = nl->node;
				if (nnl == NULL) {
					nnl = nn1;
	...
	while (nl) {
		...
		if (nl->node->indegree == 0 && nl->node->outdegree != 0) {
			nn2 = calloc(1, sizeof(struct gml_nlist));
			if (nn2) {
				nn2->node = nl->node;
				if (nnl == NULL) {
					nnl = nn2;
	...
	while (nl) {
		...
		if (nl->node->indegree != 0 && nl->node->outdegree != 0) {
			nn3 = calloc(1, sizeof(struct gml_nlist));
			if (nn3) {
				nn3->node = nl->node;
				if (nnl == NULL) {
					nnl = nn3;
	...
	while (nl) {
		...
		if (nl->node->indegree != 0 && nl->node->outdegree == 0) {
			nn4 = calloc(1, sizeof(struct gml_nlist));
			if (nn4) {
				nn4->node = nl->node;
				if (nnl == NULL) {
					nnl = nn4;
...
static void ylevels(struct gml_graph *g)
{
	...
	g->startnodes = calloc(1, g->nstartnodes * sizeof(int));
...
static void add_new_dummynode(struct gml_graph *g, int foundid)
{
	...
	lnll = calloc(1, sizeof(struct gml_nlist));
	if (lnll == NULL) {
		free(node);
		return;
	}
...
static void nodecounts(struct gml_graph *g)
{
	...
	g->nnodes_of_level = calloc((g->maxlevel + 1), sizeof(int));
